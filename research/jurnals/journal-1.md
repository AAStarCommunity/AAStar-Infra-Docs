...
- **Gas支付的必要性和Usability问题：** 本文详细介绍了区块链Gas支付中各种常见的用户体验问题：
	...
	- **Perceived Ease of Use（感知易用性）**：用户初次使用系统的感知，多数区块链系统给人的感知是复杂、晦涩、昂贵、繁琐、不安全、不流畅。
- **现有对策的局限性：** 现有系统在**可用性、认知负荷、新技术接受度**等方面存在局限性，并且现有的**中心化Gas支付系统的问题：**
...
- **拟议系统的关键要求：** 设计一个易用、安全、高性价比和去中心化的Gas支付系统的关键要求：
    - **安全:** 基于AirAccount的二次加密技术，采用用户的指纹进行交易加密和验证，确保数据安全。
	- **隐私：** 节点是基于DePIN的基础服务，运营于TEE之上，无多余任何动作，确保隐私。
    - **可用：** 基于SDSS API服务，任何dApps都可以无许可使用RestFull方式调用Gas支付服务API。
	- **易用：** 基于OpenCards，任何社会都可以发行自己的Cards和PNTs积分来做Gas Token，从而用户无感支付Gas
	- **低成本：** 基于OpenCards协议，任何人可以穿梭于多个社区，通过优势完成任务获得积分，从而实现低成本/负成本支付Gas。
    - **高效**：基于去中心化的海量服务和Token激励机制，可以实现快速、高效的Gas支付
    - **竞争**：基于竞争式报价，可以获得更低的Gas成本，以及基于成功率的Reputation排名机制来保证服务质量。
    - **开放开源无许可：** 任何社区和个人都可以获取代码、自由部署和运行Gas支付服务并加入SuperPaymaster体系。
- **SuperPaymaster 信用和信任模型：** 该模型涉及参与者（普通用户，社区，节点运营方（多种服务者），Bundler/RPC，第三方Swap）和交互行为及意图、影响因素平衡机制。
    - “节点机制：得益基于开源社区的开放无许可机制，任何节点运行者/验证者都可以在可信发行者注册表中注册，该注册表记录在公共或公共许可区块链中，基于唯一的公私钥对和链上验证机制，运行基于BLS的门限签名，从而实现去中心化、安全、高效、低成本的SuperPaymaster服务。”
	- “Reputation机制：核心的服务指标：API服务Route路由、成功率、响应速率和Stake，构成了基础的信任模型，基于客观数据的积累和统计，选择最可信可靠低价的Gas代付服务“
	- “信任模型：更好的服务-->更高的Stake-->更多的用户-->更稳定的可持续-->更高的声誉-->更多的服务：高速响应，低价服务，安全稳定，可持续；基于这个信任模型，构建稳定可靠的去中心Gas支付服务“
- **SuperPaymaster 系统流程工作流：** 本文详细介绍了系统中涉及的步骤，包括：
    - 用户使用AirAccount注册账户（可以新建合约账户或者使用已有的EOA）。
    - 用户和dApps交互，构建交易数据，使用指纹进行二次加密。
...
- **SuperPaymaster用户典型交互流程：** 本文详细介绍了系统中涉及的步骤，包括：
	...
	- 用户在dApp中访问商品页面或者转账、NFT Claim，选择商品并点击“购买”，输入自己的账户积分或者使用EOA支付USDT
	- 交易成功，如同淘宝购物一致的体验，做到了持有积分和NFT Cards，可以无感支付Gas
	- 交易成功后，用户可以查看交易记录，包括交易ID、交易时间、交易金额、交易状态等信息
	- 用户可以查看自己的积分余额、NFT Cards余额、积分历史记录、NFT Cards历史记录等信息
- **中心化Gas支付服务用户服务风险和应对：**	
	- 大部分dApps因为集成中心化的Gas支付方案技术成本高、开发周期长，并且因为合约账户的Gas成本较高，目前普及率不高。
	- 中心化验证节点：有操纵交易，抢跑，MEV被提取，三明治攻击，用户交易隐私泄露的风险；
	- 常规的流程和SuperPaymaster的系统流程区别不大（都是基于ERC4337协议），核心区别是中心化服务。
	- 中心化Gas代付服务：因为地区和法律不同，客户交易和数据可能会泄露给第三方，因为法律审查等原因。 
	- 中心化Gas代付服务：只接受自己设定的Gas Token，不会介绍第三方和其他社区的ERC20 Token。
	- 中心化Gas代付服务：因为高度垄断（全球Top3占据了50%以上交易量），所以价格在长期看有垄断风险，进而造成用户交易成本上升。
...
- **针对易用性的对策：** 本文解释了 SuperPaymaster 如何解决易用性问题：
    - **ENS访问动态服务：** 任何基于SuperPaymaster构建的Gas支付服务，都可以通过遵守SDSS标准，进行命名、API定义、路由更新、错误处理、上下限状态更新来组成稳定的服务网络。
    - **保障成功率/信用机制：** 只有提升成功率，才能获得更高的声誉，从而吸引更多用户。
    - **低成本/竞价模式：** 通过竞争式报价，选择低价的支付Gas服务。
    - **低成本/社区Token：** 因为可以自行运行Gas服务，还可以加入OpenCards来发现自己社区的加油卡，因此可以低成本的支付Gas。
	- 基于NFT的自动识别：** 通过NFT/SBT的自动识别，可以实现基于积分的无感支付，等同于Web2应用体验。
- **安全增强：** 采用了AirAccount的二次加密技术，采用用户的指纹进行交易加密和验证，确保数据安全，同时节点使用了TEE技术，确保你的数据不会泄露，交易不会被抢跑。
- **与相关工作的比较：** 本文将 SuperPaymaster 与现有的基于区块链的Gas支付解决方案进行了比较，突出了安全和隐私、成本和开放，易用性和效率几个方面，从而解决了其他方法中发现的局限性（例如，限制缺乏数据保密机制、缺乏二次验证机制、缺乏竞争式报价机制、缺乏开放的社区Token机制等）。
